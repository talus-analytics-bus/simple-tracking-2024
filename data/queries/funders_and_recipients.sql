WITH funders_and_recipients AS (
...
ranked_funders_recipients AS (
    SELECT
        funder,
	year,
        recipient,
        total,
        ROW_NUMBER() OVER (PARTITION BY recipient, year ORDER BY total DESC) AS rank
    FROM
        funders_and_recipients
)
SELECT
    recipient,
    year,
    rank,
    funder AS top_funder,
    total
FROM
    ranked_funders_recipients
WHERE
    rank <= 10;
